...
router.get("/", async (req, res) => {
    try {
                    ...
                    downtime: {
                        $dateDiff: {
                            startDate: {
                                $cond: {
                                    if: {
                                        $lte: ["$entryDate", new Date("Mon, 01 May 2023 00:00:00 GMT")],
                                    },
        ...
        // let downtimes = await Downtime.find().populate("equipment");
...
router.post("/getAnalytics", async (req, res) => {
    ...
    try {
        let downtimes = await Downtime.find({
            $or: [
                {
        ...
        let len_moveFromWorkshop = moveFromWorkshop?.length > 0 ? moveFromWorkshop?.length : 1;
        ...
        let len_stillInWorkshop = stillInWorkshop.length > 0 ? stillInWorkshop.length : 1;

        stillInWorkshop?.forEach(r => {
            avgInWorkshop = avgInWorkshop + moment().diff(moment(r.dateToWorkshop), "hours");
        });

        return res.send({
            avgInWorkshop: (avgInWorkshop / len_stillInWorkshop).toFixed(2),
            avgFromWorkshop: _.round(avgFromWorkshop / len_moveFromWorkshop, 1).toFixed(2),
            avgHours: _.round(
                _.mean([avgFromWorkshop / len_moveFromWorkshop, avgInWorkshop / len_stillInWorkshop]),
                1
            ).toFixed(2),
        });
    } catch (err) {}
...
async function getAvgDowntime(startDate, eqType) {
    try {
        let pipeline = [
            {
                $match: {
                    $or: [
                        {
                            dateFromWorkshop: {
                                $gte: new Date(moment(startDate).format("YYYY-MM-DD").toString()),
                            },
                        },
                        {
        ...
        let avgDowntimeObj = await Downtime.aggregate(pipeline);
