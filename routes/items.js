...
router.get("/items", async (req, res) => {
    ...
    if (pageSize && currentPage) itemsQuery.skip(pageSize * (currentPage - 1)).limit(pageSize);
...
router.post("/items", async (req, res) => {
    const { itemPart, uom, itemCategory } = req.body.payload;
...
router.put("/items/:id", async (req, res) => {
    const { id, itemPart, uom, itemCategory } = req.body.payload;
    ...
    if (!items) return res.status(404).send("The Items can not be found");
