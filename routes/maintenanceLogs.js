...
import { MaintenanceLogs } from "../models/maintenanceLog";
...
router.post("/maintenance/logs", async (req, res) => {
    const { entryDate, driver, carPlate, mileages, location, status } = req.body.payload;

    const jobCards = await MaintenanceLogs.find();

    // Checking if it's still in the repair mode
    const stillInRepair = jobCards.find(item => {
        if (item.plate.value == carPlate.value && item.status == "Checking") return item;
    });

    if (stillInRepair)
        return res.status(400).json({ message: "The equipment is still in repair or Issues with Mileages" });
    ...
    if (mileages && mileages.length > 0 && lowMileages)
        return res.status(400).json({ message: "Mileages input are low to the previous" });
...
router.put("/maintenance/logs/:id", async (req, res) => {
    ...
    const jobCard = await MaintenanceLogs.findOneAndUpdate(
        { jobCard_Id: req.params.id },
        {
    ...
    if (!jobCard) return res.status(404).send("The Job Card with the given ID was not found");
