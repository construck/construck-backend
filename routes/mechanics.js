...
import { Mechanics } from "../models/mechanic";
...
router.get("/mechanics", async (req, res) => {
    ...
    if (pageSize && currentPage) mechanicsQuery.skip(pageSize * (currentPage - 1)).limit(pageSize);
...
router.put("/mechanics/:id", async (req, res) => {
    ...
    if (!mechanics) return res.status(404).send("The Mechanics can not be found");
