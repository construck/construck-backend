...
import { Mechanicals } from "../models/mechanical";
...
router.get("/mechanicals", async (req, res) => {
    ...
    if (pageSize && currentPage) mechanicalsQuery.skip(pageSize * (currentPage - 1)).limit(pageSize);
...
router.post("/mechanicals", async (req, res) => {
    const { service } = req.body.payload;
...
router.put("/mechanicals/:id", async (req, res) => {
    ...
    const mechanicals = await Mechanicals.findByIdAndUpdate(
        req.params.id,
        {
            SERVICE: service,
        },
    ...
    if (!mechanicals) return res.status(404).send("The Mechanicals can not be found");
