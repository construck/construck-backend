...
import assetAvblty from "../models/assetAvailability";
import { User } from "../models/users";
import logData from "../models/logs";
import eqData from "../models/equipments";
import prjData from "../models/projects";
...
router.get("/v3", async (req, res) => {
    try {
        let workList = await Work.find({ workStartDate: { $gte: "2022-07-01" } })
            .select(
        ...
        let workList = await Work.find({
            $or: [
...

                {
                    ...
                    workStartDate: {
                        $gte: moment(startDate),
                        $lte: moment(endDate).add(23, "hours").add(59, "minutes").add(59, "seconds"),
                    },
                },
            ],
        })
...
router.get("/filtered/:page", async (req, res) => {
    ...
    try {
        let fullWorkList = await Work.find(query).select(`workStartDate`);

        let dataCount = fullWorkList.length;

        let workList = await Work.find(query)
            .select(
    ...
    try {
        ...
        let listToSend = workList
            .filter(
                w =>
                    w.siteWork === false ||
                    (w.siteWork === true && (w.status === "in progress" || w.status === "on going")) ||
                    (w.siteWork === true &&
        ...
        let l = listToSend.map(w => {
            ...
            if (w.siteWork && w.status !== "stopped" && w.status !== "recalled") {
                ...
                let dateNotPosted = datesArray.filter(
                    d => !_.includes(datesPostedDatesOnly, d) && !_.includes(datesPendingPosted, d)
                );
                ...
                datesPosted.map(dP => {
                    siteWorkList.push({
                        ...
                        startIndex: w.startIndex
                            ...
                            : //  ? w.startIndex
                              "0.0",
                        millage: parseFloat(w.equipment.millage ? w.equipment.millage : 0).toFixed(2),
                        duration:
                            dP.uom == "hour"
                ...
                if (w.siteWork === true) {
                    ...
                    datesPosted.map(dP => {
                        siteWorkList.push({
                            ...
                            duration: dP.uom === "hour" ? dP.duration / (1000 * 60 * 60) : dP.duration,
                            status: dP.status,
                            project: w.project,
                            createdOn: w.createdOn,
                            equipment: w.equipment,
                    ...
                    work = {
                        ...
                        duration: w.equipment.uom === "hour" ? w.duration / (1000 * 60 * 60) : w.duration,
                        status: w.status,
                        project: w.project,
                        createdOn: w.createdOn,
                        equipment: w.equipment,
...
router.get("/detailed/:canViewRevenues", async (req, res) => {
    ...
    let { startDate, endDate, searchText, project, userType, userProject, userProjects } = req.query;
    ...
    try {
        ...
        let workList = await Work.aggregate(pipeline);
            ...
            if (w.siteWork && w.status !== "stopped" && w.status !== "recalled") {
                ...
                datesPosted.map(dP => {
                    if (
                        ...
                        siteWorkList.push({
                            ...
                            "Duration (HRS)":
                                w.equipment?.uom === "hour" ? _.round(dP.duration / (60 * 60 * 1000), 2) : 0,
                            ...
                            ...((canViewRevenues === "true" || canViewRevenues === true) && {
                                "Projected Revenue": w.equipment?.uom === "hour" ? dP?.rate * 5 : dP?.rate,
                                "Actual Revenue":
                                ...
                                "Vendor payment":
                                    w.equipment?.uom === "hour"
                                        ? _.round(dP.duration / (60 * 60 * 1000), 2) * w?.equipment?.supplierRate
                            ...
                            "Driver contacts": w.driver?.phone,
                            "Target trips": w.dispatch?.targetTrips ? w.dispatch?.targetTrips : 0,
                ...
                dateNotPosted.map(dNP => {
                    if (
                        ...
                        siteWorkList.push({
                            ...
                            ...((canViewRevenues === "true" || canViewRevenues === true) && {
                                "Projected Revenue":
                                    w.equipment?.uom === "hour" ? w.equipment?.rate * 5 : w.equipment?.rate,
                                "Actual Revenue": 0,
                                "Vendor payment": 0,
                            }),
                ...
                if (
                    ...
                    work = {
                        ...
                        "Equipment Type": w.equipment?.eqDescription,
                        "Unit of measurement": w.equipment?.uom,
                        "Duration (HRS)": w.equipment?.uom === "hour" ? _.round(w.duration / (60 * 60 * 1000), 2) : 0,
...
router.get("/monthlyRevenuePerProject/:projectName", async (req, res) => {
    ...
    try {
        let monthlyRevenues = await Work.aggregate(pipeline);
...
router.get("/validatedList/:projectName", async (req, res) => {
    let { projectName } = req.params;
    let { month, year } = req.query;
    let result = await getValidatedListByProjectAndMonth(projectName, month, year);
...
router.get("/nonValidatedList/:projectName", async (req, res) => {
    let { projectName } = req.params;
    let { month, year } = req.query;
    let result = await getNonValidatedListByProjectAndMonth(projectName, month, year);
...
router.get("/detailed/monthlyRevenuePerProject/:projectName", async (req, res) => {
    let { projectName } = req.params;
    let { status } = req.body;
...
router.post("/", async (req, res) => {
    try {
        let workToCreate = new Work(req.body);
        ...
        equipment.assignedDate = moment(req.body?.workStartDate).format("YYYY-MM-DD");
        equipment.assignedEndDate = moment(req.body?.workEndDate).format("YYYY-MM-DD");
        ...
        workToCreate.projectedRevenue = siteWork ? revenue * workDurationDays : revenue;
        ...
        } Muhawe akazi kuri ${req.body?.project?.prjDescription} taliki ${moment(req.body?.workStartDate).format(
            "DD-MMM-YYYY"
        )} - ${req.body?.dispatch?.shift}, muzakoresha ${
...
router.post("/mobileData", async (req, res) => {
    try {
        ...
        let workToCreate = new Work(bodyData);
        ...
        if (equipment.eqStatus === "standby") {
            ...
            let workDurationDays = moment(bodyData.endTime).diff(moment(bodyData.startTime)) / MS_IN_A_DAY;
...
router.post("/getAnalytics", async (req, res) => {
    let { startDate, endDate, status, customer, project, equipment, owner } = req.body;

    let total = 0;
    let totalRevenue = 0;
    let projectedRevenue = 0;
    let totalDays = 0;
    let daysDiff = _.round(moment(endDate).diff(moment(startDate)) / MS_IN_A_DAY, 0);
    try {
        let workList = await Work.find({
            // status:
            //   status === "final"
        ...
        if (workList && workList.length > 0) {
            ...
            if (owner) {
                list = list.filter(w => {
                    ...
                    if (owner.toLowerCase() === "construck") return ownerLowercase === "construck";
                    else if (owner.toLowerCase() === "hired") return ownerLowercase !== "construck";
                    else return true;
                });
            }
            ...
            list.map(w => {
                ...
                let case1 =
                    ...
                    moment(w.workEndDate).diff(moment(endDate).add(23, "hours").add(59, "minutes").add(59, "seconds")) <
                        0;
                ...
                if (case1) {
                    ...
                    //day diff = PEnd - RangeStart
                    daysDiff = _.round(moment(w.workEndDate).diff(moment(startDate), "days"), 0);
                } else if (case3) {
                    //days diff = RangeEnd - RangeStart
                    daysDiff = _.round(moment(endDate).diff(moment(startDate), "days"), 0);
                } else if (case4) {
                    {
                        //days diff = PEnd-PStart
                        daysDiff = _.round(moment(w.workEndDate).diff(moment(w.workStartDate), "days"), 0);
                    }
                } else if (case5) {
                    //days diff = RangeEnd - PStart
                    daysDiff = _.round(moment(endDate).diff(moment(w.workStartDate), "days"), 0);
                } else {
                    daysDiff = _.round(moment(endDate).diff(moment(startDate), "days"), 0);
                }
                    ...
                    datesPosted?.map(p => {
                        if (
                            ...
                            if (w.status !== "recalled") {
                                projectedRevenue =
                                    projectedRevenue +
                                    parseInt(w.equipment?.uom === "hour" ? w.equipment?.rate * 5 : w.equipment?.rate);
                                logs.push({
                                    seq: 1,
                                    id: w._id,
                                    date: p.date,
        ...
        let workListByDay = await Work.find({ uom: "day" }).and([
            {
                "dispatch.date": {
                    $gte: startDate,
                    $lte: moment(endDate).add(23, "hours").add(59, "minutes").add(59, "seconds"),
        ...
        // let dispatches = await Work.find({
...
router.put("/:id", async (req, res) => {
    ...
    try {
        let updatedWork = await Work.findOneAndUpdate({ _id: id }, req.body);
...
router.put("/approveDailyWork/:id", async (req, res) => {
    let { id } = req.params;
    let { postingDate, approvedBy, approvedRevenue, approvedDuration, approvedExpenditure } = req.body;

    try {
        let workRec = await Work.findById(id);

        let _approvedRevenue = workRec.approvedRevenue ? workRec.approvedRevenue : 0;
        let _approvedExpenditure = workRec.approvedExpenditure ? workRec.approvedExpenditure : 0;
        let _approvedDuration = workRec.approvedDuration ? workRec.approvedDuration : 0;

        let work = await Work.findOneAndUpdate(
            {
                _id: id,
                $or: [
                    {
            ...
            },
            {
                $set: {
                    ...
                    approvedExpenditure: _approvedExpenditure + parseFloat(approvedExpenditure),
                },
            }
        );
...
router.put("/rejectDailyWork/:id", async (req, res) => {
    let { id } = req.params;
    let { postingDate, rejectedBy, rejectedRevenue, rejectedDuration, rejectedExpenditure, reason } = req.body;

    try {
        let workRec = await Work.findById(id);

        let _rejectedRevenue = workRec.rejectedRevenue ? workRec.rejectedRevenue : 0;
        let _rejectedExpenditure = workRec.rejectedExpenditure ? workRec.rejectedExpenditure : 0;
        let _rejectedDuration = workRec.rejectedDuration ? workRec.rejectedDuration : 0;
        ...
        let work = await Work.findOneAndUpdate(
            ...
            },
            {
                $set: {
                    ...
                    rejectedExpenditure: _rejectedExpenditure + parseFloat(rejectedExpenditure),
                },
            }
        );
        ...
        if (receipts.length > 0) {
            await sendEmail("appinfo@construck.rw", receipts, "Work Rejected", "workRejected", "", {
...
router.put("/releaseValidated/:projectName", async (req, res) => {
    ...
    try {
        ...
        const startOfMonth = moment().startOf("month").format(`${year}-${month}-DD`);
        const endOfMonth = moment()
            .endOf("month")
            .format(`${year}-${month}-${moment(`${year}-${month}-01`).daysInMonth(month)}`);

        console.log(startOfMonth, endOfMonth);
        let q1 = await Work.updateMany(
            {
                siteWork: false,
                "project.prjDescription": projectName,
                status: "validated",
        ...
        let q2 = await Work.updateMany(
            ...
            },
            {
                arrayFilters: [
                    {
                        $or: [{ "elemX.date": { $gte: startOfMonth } }, { "elemX.date": { $lte: endOfMonth } }],
...
router.put("/recall/:id", async (req, res) => {
    ...
    try {
        ...
        let worksInProgress = await Work.find({
            "equipment._id": eqId,
        ...
        if (worksInProgress.length <= 1) {
            ...
            equipment.assignedToSiteWork = worksInProgress[0].equipment.assignedToSiteWork;
            work.equipment = equipment;
            if (equipment) await equipment.save();
        }
            ...
            if (work.siteWork) {
                let dailyWork = {
                    day: moment(postingDate).isValid()
                        ...
                        : moment(postingDate, "DD.MM.YYYY").diff(moment(work.workStartDate), "days"),
...
router.put("/stop/:id", async (req, res) => {
    ...
    let { endIndex, tripsDone, comment, moreComment, postingDate, stoppedBy } = req.body;
    ...
    try {
        ...
        if (
            ...
            let eqBusyWorks = await Work.find({
            ...
            if (work.siteWork) {
                ...
                equipment.millage = endIndex || startIndex !== 0 ? parseInt(endIndex) : parseInt(startIndex);
                ...
                if (uom === "day") {
                    ...
                    if (comment === "Should neve happen") {
                        ...
                        let durationRation = duration >= 5 ? 1 : _.round(duration / targetDuration, 2);
                        dailyWork.duration = duration / HOURS_IN_A_DAY;
                ...
                work.startIndex = endIndex || startIndex !== 0 ? parseInt(endIndex) : parseInt(startIndex);
                ...
                let equipment = await eqData.model.findById(work?.equipment?._id);
                equipment.eqStatus = eqBusyWorks.length >= 1 ? "dispatched" : "standby";
                equipment.assignedDate = eqBusyWorks.length >= 1 ? equipment.assignedDate : null;
                equipment.assignedShift = eqBusyWorks.length >= 1 ? equipment.assignedShift : "";
                equipment.assignedToSiteWork = eqBusyWorks.length >= 1 ? equipment.assignedToSiteWork : false;
                equipment.millage = endIndex || startIndex !== 0 ? parseInt(endIndex) : parseInt(startIndex);
                ...
                work.endIndex = endIndex || startIndex !== 0 ? parseInt(endIndex) : parseInt(startIndex);
...
router.put("/resetStartIndices", async (req, res) => {
    try {
        let updates = await Work.updateMany({
            $set: {
                startIndex: 0,
                endIndex: 0,
            },
...
router.put("/swamend/:id", async (req, res) => {
    let { id } = req.params;
    let { tripsDone, comment, moreComment, postingDate, prevDuration, prevTotalRevenue, prevTotalExpenditure } =
        req.body;
    ...
    try {
        ...
        work.totalExpenditure = currentTotalExpenditure - prevTotalExpenditure + expenditure;
        work.moreComment = moreComment;
...
router.post("/gethoursperdriver/", async (req, res) => {
    ...
    try {
        ...
        let refinedData = works
            .map(w => {
                return {
                    "Main Driver": w.driverDetails[0]?.firstName + " " + w.driverDetails[0]?.lastName,
                    Drivers: w.assistantDetails,
                    Phone: w.driverDetails[0]?.phone,
                    "Total Duration": w._id.uom === "day" ? w.totalDuration : w.totalDuration / (1000 * 60 * 60),
...
router.put("/driverassistants/", async (req, res) => {
    try {
        let driversData = await Work.find({ driver: { $ne: null } }, { "dispatch.drivers": 1 });
        let allAssistants = [];
...
router.post("/reports/generate", WorkController.captureDispatchDailyReport)
...
async function getValidatedRevenuesByProject(prjDescription) {
    ...
    try {
        let validatedJobs = await Work.aggregate(pipeline);
...
async function getNonValidatedRevenuesByProject(prjDescription) {
    ...
    try {
        let nonValidatedJobs = await Work.aggregate(pipeline);
...
async function getNonValidatedListByProjectAndMonth(prjDescription, month, year) {
    let pipeline = [
        {
            $match: {
                "project.prjDescription": prjDescription,
    ...
    try {
        let jobs = await Work.aggregate(pipeline);
...
export async function getNonValidatedListByDay(prjDescription, transactionDate) {
    let pipeline = [
        {
            $match: {
                "project.prjDescription": prjDescription,
...
export async function updateCustomerRecord(oldCustomerName, newCustomerName) {
    try {
        return await Work.updateMany(
            {
...
export async function getListOfEquipmentOnDuty(startDate, endDate, shift, siteWork) {
    ...
    let pipeline_oneDayWork = [
        ...
        },
        {
            $match: {
                ...
                workDate2: {
                    $eq: moment(startDate).utcOffset(0).set({ hour: 0, minute: 0, second: 0, millisecond: 0 }).toDate(),
                },
                "dispatch.shift": {
                    $eq: shift,
                },
    ...
    return Work.aggregate(pipeline_siteWork);
}

export async function stopWork(id, endIndex, tripsDone, comment, moreComment, postingDate, stoppedBy, duration) {
    try {
        let work = await Work.findById(id)
            .populate("project")
            .populate("equipment")
        ...
        if (work.siteWork) {
            ...
            let worksAfterEffectiveDate = dailyWorks?.filter(d => moment(d?.date).isSameOrAfter(moment(postingDate)));

            worksAfterEffectiveDate.map(async dailyWork => {
                let currentTotalRevenue = work.totalRevenue;
                let currentDuration = work.duration;
                let currentTotalExpenditure = work.totalExpenditure;
                duration = equipment?.uom == "hour" ? dailyWork?.duration / 3600000 : dailyWork?.duration;
                // work.status = workEnded ? "stopped" : "on going";
                if (duration > DURATION_LIMIT) duration = DURATION_LIMIT;
