...
import { Downtime } from "../models/downtimes";
import workData from "../models/works";
import findError from "../utils/errorCodes";
import mongoose from "mongoose";
import { getListOfEquipmentOnDuty, stopWork } from "./works";
    ...
    try {
        ...
        res.status(200).send({
            ...
            available: equipments.filter(w => {
                return (w.eqStatus === "standby" || w.eqStatus === "dispatched") && w.eqOwner === "Construck";
            }).length,
...
router.put("/makeAvailable/:id", async (req, res) => {
    ...
    try {
        ...
        let _eqDowntime = await Downtime.findOne({
            equipment: id,
        ...
        if (_eqDowntime) {
            ...
            _eqDowntime.durationInWorkshop = moment().diff(moment(_eqDowntime.dateToWorkshop), "hours");
            _eqDowntime.save();
        } else {
            const data = {
        ...
        const dateData = await Asset.findOne({ date: today });
        ...
        if (dateData) {
            ...
            await Asset.create(dateDataToSave);
        }
...
router.get("/utilization/all", EquipmentController.getEquipmentUtilization)
router.get("/utilization/date/:date", EquipmentController.getEquipmentUtilizationByDate)
router.get("/utilization/average/:startdate/:enddate", EquipmentController.getEquipmentUtilizationAverageByDates)
router.post("/utilization", EquipmentController.captureEquipmentUtilization)
